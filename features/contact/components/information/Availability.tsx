...
export default function Availability({ baseInfo }: IBaseInfo) {
  ...
  return (
    <div className="inline-flex items-center gap-2 px-3 py-1 bg-green-500/10 border border-green-500/20 rounded-full mb-4">
      <div
        className={`w-2 h-2 ${availability.toLocaleLowerCase() === 'available' ? 'bg-green-400' : 'bg-red-400'} rounded-full animate-pulse`}
      />
      <span
        className={`text-sm font-medium ${availability.toLocaleLowerCase() === 'available' ? 'text-green-300' : 'text-red-300'}`}>
        {availability.toLocaleUpperCase()}
      </span>
    </div>
  );
}
